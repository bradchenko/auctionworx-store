    ...
    public class ListingController : AuctionWorxController
    {
        ...
        public ActionResult ConfirmActionModal(int ListingID)
        {
            ...
            try
            {
                ...
                bool isCcReqExempt = (loggedOnUser != null && loggedOnUser.Properties != null)
                    ? loggedOnUser.Properties.GetPropertyValue(StdUserProps.CreditCardRequiredExempt, false) : false;
                if (!isCcReqExempt && SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(listing.OwnerUserName, this.FBOUserName()))
                {
        ...
        public ActionResult MakeOfferModal(int ListingID)
        {
            ...
            if (!isCcReqExempt && User.Identity.IsAuthenticated && SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(listing.OwnerUserName, this.FBOUserName()))
            {
                PrepareNeutralMessage(Messages.CreditCardRequiredForBuyers);
                //return RedirectToAction(Strings.MVC.AddCardAction, Strings.MVC.AccountController, new { returnUrl = Request.Url.PathAndQuery });
                ViewData["CCRequired"] = true;
        ...
        public ActionResult ConfirmOfferModal(int ListingID)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    bool isCcReqExempt = (loggedOnUser != null && loggedOnUser.Properties != null)
                        ? loggedOnUser.Properties.GetPropertyValue(StdUserProps.CreditCardRequiredExempt, false) : false;
                    if (!isCcReqExempt && SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin") && !this.HasUnexpiredCardOnFile(newOffer.ListingOwnerUsername, this.FBOUserName()))
                    {
                        PrepareNeutralMessage(Messages.CreditCardRequiredForBuyers);
                        //return RedirectToAction(Strings.MVC.AddCardAction, Strings.MVC.AccountController, new { returnUrl = Request.Url.PathAndQuery });
                        ViewData["CCRequired"] = true;
        ...
        public JsonResult SubmitOffer(int ListingID)
        {
            ...
            else
            {
                ...
                bool isCcReqExempt = (loggedOnUser != null && loggedOnUser.Properties != null)
                    ? loggedOnUser.Properties.GetPropertyValue(StdUserProps.CreditCardRequiredExempt, false) : false;
                if (!isCcReqExempt && SiteClient.BoolSetting(SiteProperties.RequireCreditCardForBuyers) && !User.IsInRole("Admin"))
                {
                    var listing = ListingClient.GetListingByIDWithFillLevel(User.Identity.Name, ListingID, ListingFillLevels.None);
                    if (listing != null && !this.HasUnexpiredCardOnFile(listing.OwnerUserName, this.FBOUserName()))
                    {
